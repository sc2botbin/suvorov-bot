...
float CalcConsumptionRate::operator()(float sum, const sc2::Unit* unit_) const
{
    ...
    switch (unit_->unit_type.ToType())
    {
        ...
        case sc2::UNIT_TYPEID::TERRAN_BARRACKSTECHLAB:
        {
            ...
            {
                return sum + 2.0f;
                // just add 2.0 for all others for now
            }
...
void QuarterMaster::OnStep(Builder* builder_)
{
    ...
    // Do we have an order to build the supply type? If so, we will start a waiting period of 10 seconds.
    std::list<Order> orders = builder_->GetOrders();
    if (!orders.empty() && orders.begin()->unit_type_id == gHub->GetCurrentSupplyType())
    {
    ...
    // We expect to use more supply than we currently have, so build a supply type and enter a waiting period.
